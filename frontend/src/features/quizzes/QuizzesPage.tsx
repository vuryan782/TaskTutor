...
export default function QuizzesPage() {
  ...
  const score = activeQuiz
    ? Math.round(
        (answers.filter((ans, i) => ans === activeQuiz.questions[i].answerIndex)
          .length /
      ...
      <div className="space-y-6">
        ...
        <div className="max-w-lg bg-[#16161e] rounded-xl border border-[#2a2a3a] p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <p className="font-semibold text-[#e8e8ed]">
                AI Question Generator
              </p>
              <p className="text-xs text-[#5c5c72]">
                Gemini will generate 10 questions instantly
              </p>
            </div>
          </div>
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto">
        ...
        <div className="bg-[#16161e] rounded-xl border border-[#2a2a3a] p-6">
          <p className="text-lg font-semibold text-[#e8e8ed] mb-6">
            {q.question}
          </p>
          ...
          <div className="space-y-3">
            {q.options.map((option, i) => {
              let style =
                "border-[#2a2a3a] bg-[#1c1c27] hover:bg-[#222233] text-[#e8e8ed]";
              if (confirmed) {
                if (i === q.answerIndex) {
              ...
              return (
                ...
                >
                  <span>{option}</span>
                  {confirmed && i === q.answerIndex && (
                    <CheckCircle className="w-5 h-5 text-[#4ade80] flex-shrink-0" />
                  )}
                  {confirmed && i === selected && i !== q.answerIndex && (
          ...
          {confirmed && (
            <div
              className={`mt-4 px-4 py-3 rounded-lg text-sm font-medium ${isCorrect ? "bg-[#4ade80]/10 text-[#4ade80]" : "bg-[#ef4444]/10 text-[#ef4444]"}`}
            >
              {isCorrect
                ? "Correct!"
    ...
    return (
      <div className="space-y-6 max-w-2xl mx-auto">
        <div className="bg-[#16161e] rounded-xl border border-[#2a2a3a] p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-[#e8e8ed] mb-1">
            Quiz Complete!
          </h2>
          ...
          {savingResult ? (
            <p className="text-xs text-[#7c5cfc] mt-2">
              Saving result to your progress...
            </p>
          ) : (
            <p className="text-xs text-[#4ade80] mt-2">
              Result saved to your Progress page
            </p>
          )}
          ...
          <div className="space-y-4">
            {activeQuiz.questions.map((q, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`p-4 rounded-lg border ${wasCorrect ? "bg-[#4ade80]/10 border-[#4ade80]/20" : "bg-[#ef4444]/10 border-[#ef4444]/20"}`}
                >
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8b8b9e]">
            AI-generated quizzes tailored to your topics
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-[#7c5cfc]/20 to-[#6a4ce0]/10 border-2 border-[#7c5cfc]/30 rounded-xl p-6 hover:shadow-lg hover:shadow-[#7c5cfc]/5 transition-shadow">
          <div className="flex items-start justify-between mb-4">
            ...
            <span className="text-xs bg-[#7c5cfc]/10 text-[#7c5cfc] px-2 py-1 rounded-full">
              Active
            </span>
          </div>
          <h3 className="font-bold text-[#e8e8ed] mb-2">
        ...
        <div className="bg-[#16161e] border border-[#2a2a3a] rounded-xl p-6 hover:shadow-lg hover:shadow-black/10 transition-shadow">
          <div className="flex items-start justify-between mb-4">
            ...
            <span className="text-xs bg-[#8b8b9e]/10 text-[#8b8b9e] px-2 py-1 rounded-full">
              Draft
            </span>
          </div>
          <h3 className="font-bold text-[#e8e8ed] mb-2">
        ...
        {quizzes.map((quiz) => (
          <div
            key={quiz.id}
            className="bg-[#16161e] border border-[#2a2a3a] rounded-xl p-6 hover:shadow-lg hover:shadow-black/10 transition-shadow"
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs bg-[#4ade80]/10 text-[#4ade80] px-2 py-1 rounded-full">
                  AI
                </span>
                <button
